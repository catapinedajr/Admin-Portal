...
- June 27, 2025. Restored manual Calculate button control in DCA simulator: removed automatic calculation on input changes, users now must click Calculate button to trigger calculations and see results, maintains clean interface with conditional results display until calculation is performed, providing better user control over when calculations happen
