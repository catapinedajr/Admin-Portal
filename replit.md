...
- June 29, 2025. **COMPLETE REBRANDING TO BTC JOURNEY**: Successfully rebranded entire application from "HODLearn" to "BTC Journey" for broader market appeal and accessibility: updated browser title, header logo (simplified "BTC" design), splash screen branding with new "Learn • Grow • Succeed" tagline, affiliate disclosure text, localStorage onboarding keys (btcjourney-onboarding-completed), and seed phrase recovery simulator practice word changed from "hodlearn" to "journey" across all scenarios (12/16/24-word phrases) with updated safety messaging and hints for consistent brand alignment throughout user experience
