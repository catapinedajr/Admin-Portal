      ...
      <main className="max-w-6xl mx-auto px-4 py-6">
        ...
        {activeSection === "simulations" && (
          <div className="space-y-6">
            ...
            {/* Compact Transaction Simulator */}
            {simulationsSubTab === "transactions" && (
              <div className="space-y-4">
                <div className="text-center space-y-2">
                  <h3 className="text-xl font-bold text-white">Bitcoin Transaction Simulator</h3>
                  <p className="text-zinc-400">Practice building Bitcoin transactions step-by-step</p>
                </div>
                ...
                <Card className="bg-zinc-900 border-zinc-800">
                  <CardContent className="p-6">
                    <div className="space-y-4">
                      ...
                      <div className="grid gap-4 md:grid-cols-2">
                        <div className="space-y-2">
                          <label className="text-sm text-zinc-300">Amount (BTC)</label>
                          <input
                            type="number"
                            step="0.00000001"
                            value={transactionInputs.amount}
                            onChange={(e) => updateTransactionInput('amount', e.target.value)}
                            className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded text-white text-sm focus:border-orange-500 focus:outline-none"
                            placeholder="0.001"
                          />
                        </div>
                        <div className="space-y-2">
                          <label className="text-sm text-zinc-300">USD Value</label>
                          <div className="px-3 py-2 bg-zinc-800 border border-zinc-700 rounded text-green-400 text-sm">
                      ...
                      <div className="space-y-2">
                        <label className="text-sm text-zinc-300">Recipient Address</label>
                        <div className="flex gap-2">
                          <input
                            type="text"
                            value={transactionInputs.toAddress}
                            onChange={(e) => updateTransactionInput('toAddress', e.target.value)}
                            className="flex-1 px-3 py-2 bg-zinc-800 border border-zinc-700 rounded text-white text-sm focus:border-orange-500 focus:outline-none"
                            placeholder="Click paste to add address"
                          />
                          <Button
                            variant="outline"
                            size="sm"
                            onClick={simulatePasteFromClipboard}
                            className="border-zinc-700 text-zinc-300 hover:border-orange-500 shrink-0"
                          >
                            📋
                          </Button>
                        </div>
                      ...
                      <div className="space-y-2">
                        <label className="text-sm text-zinc-300">Fee Priority</label>
                        <select
                          value={transactionInputs.feeRate}
                          onChange={(e) => updateTransactionInput('feeRate', e.target.value)}
                          className="w-full px-3 py-2 bg-zinc-800 border border-zinc-700 rounded text-white text-sm focus:border-orange-500 focus:outline-none"
                        >
                          <option value="low">Low Priority (1-3 hours)</option>
                          <option value="standard">Standard (10-30 min)</option>
                          <option value="high">High Priority (~10 min)</option>
                        </select>
                      </div>

                      <div className="p-3 bg-zinc-800/50 rounded text-sm">
                        ...
                        <div className="flex justify-between mb-1">
                          ...
                          <span className="text-white">{getCurrentFee().cost} BTC</span>
                        </div>
                        <div className="flex justify-between border-t border-zinc-700 pt-1">
                          <span className="text-zinc-300 font-medium">Total:</span>
                          <span className="text-orange-400 font-medium">{getTransactionTotal()} BTC</span>
                        </div>
                      </div>
                      ...
                      <Button 
                        ...
                        disabled={!transactionInputs.amount || !transactionInputs.toAddress}
                      >
