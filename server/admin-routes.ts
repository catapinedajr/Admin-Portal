...
import { awardPoints, awardManualBonus } from "./rewards-service";
...
Return ONLY the post content, nothing else.`;
  ...
  app.post("/api/admin/rewards/manual-award", requireAdminAuth, async (req: AdminRequest, res: Response) => {
    try {
      ...
      // Use the centralized reward service
      const result = await awardManualBonus(userId, amount, reason || `Manual award by admin`);
      ...
      res.json({
        ...
        satoshisAwarded: result.satoshisEarned,
